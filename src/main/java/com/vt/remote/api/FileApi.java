...
public class FileApi {
     ...
     * @return 文件id及分析结果链接
     */
    public VtResult<FileScanResp> reAnalyze(String id) {
        String url = ApiEnum.RE_ANALYSE_FILE.getApiUrl(id);
        return vtRemoter.post(url, new EmptyContent(), new TypeToken<VtResult<FileScanResp>>() {});
    }
     ...
     * @return 报告
     */
    public VtResult<FileReportResp> getFileReport(String id) {
        String url = ApiEnum.GET_FILE_REPORT.getApiUrl(id);
        return vtRemoter.get(url, new TypeToken<VtResult<FileReportResp>>() {});
    }
    ...
    public VtResult<List<FileBehaviourReportResp>> getBehaviourReport(String id) {
        String url = ApiEnum.GET_BEHAVIOUR_REPORT.getApiUrl(id);
        return vtRemoter.get(url, new TypeToken<VtResult<List<FileBehaviourReportResp>>>() {});
    }
    ...
    private String chooseUrl(int size) {
        ...
        if (SizeConstant.SIZE_BOUNDARIES > size) {
            return ApiEnum.UPLOAD_LT_32.getApiUrl(null);
        }
     ...
     * @return 文件id
     */
    private VtResult<FileScanResp> upload(FileUploadParse uploadParse, String url) {
        MultipartContent content = uploadParse.uploadForm();
        return vtRemoter.post(url, content, new TypeToken<VtResult<FileScanResp>>() {});
    }
