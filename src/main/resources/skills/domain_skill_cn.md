...
- `last_dns_records (List<DnsRecord>)`: 最近解析记录。
    ...
    - **安全意义**：
        ...
        - **DNS over TLS (DoT) 异常**：如果发现域名相关解析涉及到端口 853 的通信或明确提供 DoT 服务，需注意：恶意软件常利用 DoT 加密其 C2 域名的解析过程，从而绕过企业网络基于明文 DNS 的监控与阻断策略。
...
- **[加密规避意图 (DoT 滥用)]**：恶意域名主动通过 DNS over TLS (通常端口为 853) 进行解析，试图向网络监控设备隐藏其 DNS 流量。
...
*(严格要求：必须对 JSON 中提供的所有维度的资产信息进行全量解析，严禁遗漏任何安全特征。如果报告过长请分块输出。)*
